...
export default function ResourcesPage() {
  ...
  const handleAddResource = async (e: React.FormEvent) => {
    ...
    try {
      ...
      loadResources();
      showToast('Learning resource published to library successfully!');
    } catch (err) {
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <div key={n} className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm space-y-4">
              ...
              <Skeleton className="h-8 rounded-xl pt-2" />
            </div>
          ))}
        </div>
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredResources.map((res) => (
            <StaggerItem key={res.id}>
              <MotionCard
                className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:border-[#D4A017] hover:shadow-md transition-all flex flex-col justify-between space-y-4 h-full"
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-[#E8F0FA] text-[#1E4E8C] uppercase tracking-wider">
                      Ages {res.age_range}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#6B7280] mt-1 line-clamp-2">
                      {res.description || 'Montessori tailored home practice guide & printable worksheets.'}
                    </p>
                  </div>
                </div>
