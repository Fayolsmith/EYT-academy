    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="p-6 bg-white rounded-3xl border border-gray-100 shadow-xs space-y-4">
              ...
              {submittedList.length === 0 ? (
                ...
                <StaggerContainer className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {submittedList.map((assignment) => {
                    const sub = submissions[assignment.id];
                    return (
                      <StaggerItem key={assignment.id} className="h-full">
                        <MotionCard
                          hoverEffect="lift"
                          className="bg-white rounded-3xl border-2 border-amber-200 p-6 space-y-5 shadow-xs relative overflow-hidden h-full flex flex-col justify-between"
                        >
                          ...
                          <div className="pt-2 flex justify-end">
                            <MotionButton
                              ...
                              className="px-4 py-2.5 rounded-xl bg-[#1E4E8C] text-white font-bold text-xs hover:bg-[#153763] transition-colors flex items-center gap-2 shadow-xs cursor-pointer"
                            >
                    ...
                    return (
                      ...
                      >
                        ...
                        {sub?.milestone_marked_achieved && a.milestone_name && (
                          <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 flex items-center justify-between text-emerald-900 text-xs font-bold">
                            <div className="flex items-center gap-2">
                              <Award className="w-4 h-4 text-[#D4A017] shrink-0" />
                              <span>Milestone: {a.milestone_name}</span>
                            </div>
                            <MilestoneAchievementBadge label="Achieved" isRecent={false} />
                          </div>
                        )}
      ...
      >
        <div className="p-6 sm:p-7 space-y-5 max-h-[90vh] overflow-y-auto">
          ...
          {/* Child Details Lock Display */}
          <div className="p-3.5 bg-gray-50 rounded-2xl border border-gray-200">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-[#6B7280] uppercase tracking-wider block">
                Assigning to Enrolled Student (Locked)
              </span>
              <span className="inline-flex items-center gap-1 text-[10px] font-bold text-[#1E4E8C] bg-[#E8F0FA] px-2 py-0.5 rounded-full">
                <Lock className="w-3 h-3 text-[#1E4E8C]" />
                1:1 Individualized
              </span>
            </div>

            {selectedChild ? (
              <div className="p-3.5 bg-[#F8FAFC] rounded-2xl border border-gray-200 flex items-center gap-3.5 mt-2">
      ...
      {/* ========================================================= */}
      <AnimatedModal
        isOpen={!!photoPreviewModalUrl}
        onClose={() => setPhotoPreviewModalUrl(null)}
        maxWidth="max-w-2xl"
      >
